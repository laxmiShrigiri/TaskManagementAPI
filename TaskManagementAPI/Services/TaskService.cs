...
namespace TaskManagementAPI.Services
{
    public class TaskService(AppDbContext db) : ITaskService
    {
            ...
            CreateTaskRequestDto dto)
        {
            ...
            if (dto.AssignedToUserId.HasValue)
            {
                var isMember = project.Members
                    .Any(m => m.userId == dto.AssignedToUserId.Value);
            ...
            TaskQueryParametersDto dto)
        {
            ...
            if (dto.AssignedToUserId.HasValue)
                query = query.Where(t =>
                    t.AssignedToUserId == dto.AssignedToUserId.Value);
            ...
            UpdateTaskRequestDto dto)
        {
            ...
            if (dto.AssignedToUserId.HasValue)
            {
                var isMember = await db.ProjectMembers.AnyAsync(m =>
                    m.userId == dto.AssignedToUserId.Value &&
        ...
        public async Task<TaskResponseDto> UpdateStatus(
            Guid userId,
            string role,
            Guid taskId,
            UpdateTaskStatusRequestDto dto)
        {
            ...
            if (role == nameof(UserRole.Member) &&
            ...
            if (task.Status == TaskItemStatus.Cancelled &&
                dto.Status == TaskItemStatus.Completed)
            {
                throw new BadRequestException(
                    "A cancelled task cannot be marked directly as completed.");
            }
        ...
        public async Task<PagedResult<TaskResponseDto>> GetTasksPage(
            Guid userId,
            string role,
            TaskQueryParameters query)
        {
            ...
            if (query.ProjectId.HasValue)
                taskQuery = taskQuery.Where(t =>
                    t.ProjectId == query.ProjectId.Value);
            ...
            if (query.DueBefore.HasValue)
                taskQuery = taskQuery.Where(t =>
                    t.DueDate <= query.DueBefore.Value);
            ...
            taskQuery = (query.SortBy?.ToLower()) switch
            {
                ...
                "priority" => query.SortDescending
                    ? taskQuery.OrderByDescending(t => t.Priority)
                    : taskQuery.OrderBy(t => t.Priority),

                "title" => query.SortDescending
                    ? taskQuery.OrderByDescending(t => t.Title)
                    : taskQuery.OrderBy(t => t.Title),
            ...
            string role)
        {
            ...
            if (role == nameof(UserRole.ProjectManager) &&
